...
\title{\textbf{Bridging short-ranged MACE and a full description of interatomic interactions with Fourier neural operators}}
\author{Wei Chen}
...
We do not ask whether an FNO can replace MACE. Instead, we ask whether a
global operator can learn the structured error that remains after a strong
local potential has already described short-range chemistry. We use the
decomposition
\begin{equation}
...
\begin{enumerate}
 ...
 \item We show that a slab-specific 2.5D residual matches the remote-Al sign reversal in
...
We therefore interpret the current model as
\[
...
We deposit these amplitudes with cubic B splines to obtain mesh fields
\begin{equation}
 \rho_c(\mathbf r_m)=\sum_i s_{ic}B_3(\mathbf r_m-\mathbf r_i),
 \quad\text{cf. Refs.~\citep{darden1993pme,essmann1995pme}.}
\end{equation}
We map these sources to response fields
\(\boldsymbol\phi=\mathcal G_\omega[\boldsymbol\rho;\mathbf H]\) with the
FNO and compute the scalar correction as a bilinear mesh functional,
\begin{equation}
 \Delta E_{\mathrm{FNO}}
...
post-deposition response operator. We summarize the distinction as
\[
...
\]
This flexibility could represent heterogeneous screening or response that is
...
Our test suite covers deposition, spectral energy, 2D/2.5D/3D operators,
