...
\title{\textbf{Learning nonlocal residuals in interatomic potentials with Fourier neural operators}}
\author{Wei Chen}
...
\subsection{From a dense nonlocal kernel to a Fourier multiplier}

A conventional neural network maps one finite-dimensional vector to another.
A neural operator instead maps an input field to an output field,
\(
 \mathcal G_\omega:a(\mathbf r)\mapsto u(\mathbf r)
\), with parameters shared over the spatial domain and, in the continuum
idealization, across discretizations~\citep{kovachki2023neuraloperator}. We
begin with a general linear nonlocal operator,
\begin{equation}
 (\mathcal K v)(\mathbf r)
 =\int_\Omega
 \kappa(\mathbf r,\mathbf r')v(\mathbf r')\,\mathrm d\mathbf r'.
 \label{eq:integral-operator}
\end{equation}
The value at \(\mathbf r\) collects information from every source position
\(\mathbf r'\). After discretization on \(N\) mesh points, the most general
kernel is therefore an \(N\times N\) interaction matrix. Applying it directly
requires \(\mathcal O(N^2)\) work and storing up to \(\mathcal O(N^2)\)
couplings.

\subsubsection{Translation invariance produces a convolution}

The structure of Eq.~\eqref{eq:integral-operator} simplifies when translating
the source and observation point by the same vector leaves their coupling
unchanged. The kernel then depends on their separation rather than their
absolute positions,
\(
 \kappa(\mathbf r,\mathbf r')=\kappa(\mathbf r-\mathbf r')
\), and
\begin{equation}
 (\mathcal K v)(\mathbf r)
 =\int_\Omega \kappa(\mathbf r-\mathbf r')v(\mathbf r')\,\mathrm d\mathbf r'
 \equiv (\kappa*v)(\mathbf r).
 \label{eq:translation-convolution}
\end{equation}
Thus, a translation-invariant nonlocal operator is a convolution. Coulomb
interaction in homogeneous space is a familiar example because its kernel
depends on \(|\mathbf r-\mathbf r'|\), although the FNO construction is not
restricted to Coulomb physics.

\subsubsection{Fourier modes diagonalize the convolution}

The computational advantage follows from the action of
Eq.~\eqref{eq:translation-convolution} on a single Fourier wave,
\(v(\mathbf r)=\exp(i\mathbf k\cdot\mathbf r)\). Substituting
\(\mathbf s=\mathbf r-\mathbf r'\) gives
\begin{align}
 (\mathcal K v)(\mathbf r)
 &=\int_\Omega \kappa(\mathbf s)
   \exp[i\mathbf k\cdot(\mathbf r-\mathbf s)]\,\mathrm d\mathbf s \notag\\
 &=\exp(i\mathbf k\cdot\mathbf r)
   \int_\Omega \kappa(\mathbf s)
   \exp(-i\mathbf k\cdot\mathbf s)\,\mathrm d\mathbf s \notag\\
 &=\widehat\kappa(\mathbf k)\exp(i\mathbf k\cdot\mathbf r).
 \label{eq:fourier-eigenfunction}
\end{align}
The output is the same wave multiplied by the scalar
\(\widehat\kappa(\mathbf k)\). Fourier modes are therefore eigenfunctions of a
translation-invariant convolution operator.

A general periodic field is a superposition of these waves,
\begin{equation}
 v(\mathbf r)=\sum_{\mathbf k}\widehat v(\mathbf k)
 \exp(i\mathbf k\cdot\mathbf r).
 \label{eq:fourier-expansion}
\end{equation}
Linearity and Eq.~\eqref{eq:fourier-eigenfunction} then give the convolution
theorem in the form used by the FNO,
\begin{equation}
 \widehat{\mathcal K v}(\mathbf k)
 =\widehat\kappa(\mathbf k)\widehat v(\mathbf k)
 \equiv R(\mathbf k)\widehat v(\mathbf k).
 \label{eq:fourier-multiplier}
\end{equation}
Equation~\eqref{eq:fourier-multiplier} contains three distinct quantities.
The coefficient \(\widehat v(\mathbf k)\) measures the amplitude and phase of
the input field at wavevector \(\mathbf k\). The multiplier
\(R(\mathbf k)=\widehat\kappa(\mathbf k)\) is the eigenvalue of the convolution
operator for that Fourier mode. The left-hand side is the corresponding
coefficient of the output field. If
\(\widehat v(\mathbf k)=A\exp(i\varphi)\) and
\(R(\mathbf k)=B\exp(i\psi)\), then
\[
 \widehat{\mathcal K v}(\mathbf k)=AB\exp[i(\varphi+\psi)].
\]
The magnitude \(|R(\mathbf k)|\) therefore amplifies or suppresses that
wavelength, while its phase changes the phase of the response. For a real,
even kernel, \(R(\mathbf k)\) is real and even. More general kernels can yield
complex multipliers, with conjugate symmetry ensuring a real output field.

The mode-wise multiplication is local in wavevector but global in real space.
Each coefficient \(\widehat v(\mathbf k)\) is an integral over the complete
domain, and each Fourier basis function extends across that domain. Thus,
Eq.~\eqref{eq:fourier-multiplier} does not remove nonlocal coupling. It
represents the same coupling in a basis where a translation-invariant operator
is diagonal. A single output coefficient depends only on the input coefficient
at the same \(\mathbf k\), but the reconstructed value at any mesh point
depends on all retained coefficients.

For an exact convolution, \(R(\mathbf k)\) is fixed by the physical kernel.
The FNO replaces it within a retained spectral band by trainable parameters
\(R_\theta(\mathbf k)\). It thereby learns which wavelengths are transmitted,
attenuated, or amplified. Modes outside the retained band are set to zero in
the spectral pathway, while the pointwise pathway in Eq.~\eqref{eq:fno-layer}
preserves local high-frequency information. The learned multiplier is not
automatically a pair potential, dielectric function, or Coulomb Green's
function. Such an interpretation requires independent physical diagnostics.

Applying Eq.~\eqref{eq:fourier-multiplier} consists of a fast Fourier
transform (FFT), multiplication by \(R(\mathbf k)\), and an inverse FFT.
Instead of coupling every pair of real-space points explicitly, the operator
therefore processes reciprocal-space modes independently.

The same result has a useful matrix interpretation. On a one-dimensional
periodic grid, a separation-dependent kernel produces a circulant matrix such
as
\begin{equation}
 K=
 \begin{pmatrix}
  ...
  b&c&d&a
 \end{pmatrix}.
 \label{eq:circulant-matrix}
\end{equation}
Its eigenvectors are the discrete Fourier modes, so the discrete Fourier
matrix \(F\) diagonalizes it:
\begin{equation}
 K=F^{-1}\operatorname{diag}\!\left[R(k_0),\ldots,R(k_{N-1})\right]F.
 \label{eq:circulant-diagonalization}
\end{equation}
Higher-dimensional periodic convolutions have the corresponding
block-circulant structure. With multiple fields, each diagonal scalar becomes
a small matrix that mixes channels at a fixed \(\mathbf k\), while different
spatial modes remain uncoupled within the linear spectral operation.

This transformation is especially useful for slowly decaying kernels. For
example, the three-dimensional Coulomb Green's function satisfies,
schematically,
\begin{equation}
 \frac{1}{r}\xrightarrow{\mathcal F}\frac{4\pi}{k^2},\qquad k\ne0,
 \label{eq:coulomb-fourier}
\end{equation}
with the zero mode and boundary conditions handled separately. A dense
all-to-all real-space interaction can consequently be applied through FFTs
with \(\mathcal O(N\log N)\) scaling. This is the mathematical connection
between spectral operators and reciprocal-space electrostatic methods. It
also motivates our low-wavevector \(1/k^2\) diagnostic, but it does not imply
that the learned FNO response must be electrostatic.

\subsubsection{The learned spectral response}

For a known Green's function, the multiplier in
Eq.~\eqref{eq:fourier-multiplier} can be prescribed analytically. An FNO
instead learns it from data~\citep{li2021fno}. With several feature channels,
the scalar multiplier is replaced by a trainable complex matrix for every
retained mode,
\begin{equation}
 \widehat u_b(\mathbf k)
 =\sum_a R_{ba}(\mathbf k)\widehat v_a(\mathbf k),
 \label{eq:multichannel-response}
\end{equation}
where \(a\) and \(b\) label input and output channels. The matrix can encode
different responses and couplings among latent fields without assigning them
a predetermined physical identity. Stacking spectral operations with
pointwise channel mixing and nonlinear activations extends this linear
translation-invariant starting point to a nonlinear operator.

\subsubsection{Global communication in MACE--FNO}

The connection to our residual model follows directly. Frozen MACE first maps
each local environment to an invariant descriptor \(\mathbf h_i\). A learned
source head converts these descriptors into scalar amplitudes
\(s_{ic}\), which particle--mesh assignment deposits as latent fields
\(\rho_c(\mathbf r)\). Every Fourier coefficient
\(\widehat\rho_c(\mathbf k)\) is computed from the entire mesh. It can
therefore contain contributions from two sources separated by more than the
MACE receptive field. Equation~\eqref{eq:multichannel-response} processes
their combined coefficient, and the inverse transform carries the resulting
response back to both locations. A retained Fourier basis function has global
support, so one spectral block communicates across the complete periodic
domain; retaining more modes improves spatial resolution rather than extending
the communication distance.

Full diagonalization in \(\mathbf k\) relies on translation invariance along
every transformed direction. A heterogeneous slab is approximately
translation invariant in its periodic plane but not normal to the interface.
Our 2D FNO therefore transforms the two in-plane coordinates and retains the
normal coordinate explicitly, where a learned finite-layer map mixes
information between heights. Section~\ref{sec:geometries} describes this
geometry-specific construction.

\subsection{Discrete FNO blocks, modes and channels}
...
spectral matrix \(R_\ell(\mathbf n)\) implements
Eq.~\eqref{eq:multichannel-response} at each retained mode, and all unretained
