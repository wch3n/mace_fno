...
truncated set of modes. A Fourier mode is a basis function
\(\exp(i\mathbf k\cdot\mathbf r)\) identified by a discrete reciprocal vector
\(\mathbf k\). The configured mode cutoff selects the lowest Fourier indices
on which \(R_\ell\) acts. These low-\(k\) components describe slowly varying
fields, while a larger cutoff adds shorter-wavelength structure and increases
cost. Every retained Fourier basis function is global. Increasing the mode
count therefore improves spectral resolution rather than extending the
receptive field. The pointwise path \(W_\ell v_\ell\) still carries local mesh
information outside the truncated spectral branch.

Channels instead label the components of the learned feature vector
\(v_\ell(\mathbf r)\) at each mesh point. In Eq.~\eqref{eq:fno-layer},
\(W_\ell\) mixes channels pointwise and \(R_\ell(\mathbf k)\) mixes them at
retained wavevectors. We distinguish the \(C\) latent source channels
\(\rho_c\) from the hidden channels used inside each FNO block. Source
channels provide capacity for several learned source--response components,
whereas hidden channels set the internal network width. Neither type denotes
a chemical species, atomic orbital or assigned electrostatic multipole. The
learned source-channel basis remains non-unique under equivalent latent
representations.
...
finite-\(z\) mixing, two-way lateral interlacing, D4 averaging and float64
